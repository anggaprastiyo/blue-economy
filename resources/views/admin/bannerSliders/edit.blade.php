...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.banner_slider.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.banner_sliders.update", [$bannerSlider->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title"
                           id="title" value="{{ old('title', $bannerSlider->title) }}" required>
                    @if($errors->has('title'))
                ...
                <div class="form-group">
                    <label for="text">{{ trans('cruds.banner_slider.fields.text') }}</label>
                    <textarea class="form-control ckeditor {{ $errors->has('text') ? 'is-invalid' : '' }}" name="text"
                              id="text">{!! old('text', $bannerSlider->text) !!}</textarea>
                    @if($errors->has('text'))
                        <div class="invalid-feedback">
                            {{ $errors->first('text') }}
                        </div>
                ...
                <div class="form-group">
                    ...
                    @foreach(App\Models\BannerSlider::PUBLISH_STATUS_RADIO as $key => $label)
                        <div class="form-check {{ $errors->has('publish_status') ? 'is-invalid' : '' }}">
                            <input class="form-check-input" type="radio" id="publish_status_{{ $key }}"
                                   name="publish_status"
                                   value="{{ $key }}" {{ old('publish_status', $bannerSlider->publish_status) === (string) $key ? 'checked' : '' }}>
...
@section('scripts')
                            ...
                            return loader.file
                                .then(function (file) {
                                    return new Promise(function (resolve, reject) {
                                        ...
                                        data.append('crud_id', '{{ $bannerSlider->id ?? 0 }}');
                                        xhr.send(data);
                                    });
                                })
                        }
    ...
    <script>
        Dropzone.options.featuredImageDropzone = {
            ...
            init: function () {
                @if(isset($bannerSlider) && $bannerSlider->featured_image)
                var file = {!! json_encode($bannerSlider->featured_image) !!}

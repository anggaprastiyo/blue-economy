            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ContentPage::PUBLISH_STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('publish_status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="publish_status_{{ $key }}" name="publish_status" value="{{ $key }}" {{ old('publish_status', $contentPage->publish_status) === (string) $key ? 'checked' : '' }} required>
